...
const SectionCard = ({history, sIndex, section}) => {
    const handleRedirecting = (sectionId) => {
        history.push({
            pathname: `/account/survey/${sectionId}`,
            state: {
                sIndex
            }
        });
    ...
    return (
        <div onClick={handleRedirecting.bind(this, section.id)} className='section-card'>
            <div className='section-title'>
                <strong>{section.title}</strong>
                <div className='section-period'>{section.duration} dk.</div>
            </div>
